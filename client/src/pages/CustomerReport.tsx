...
const CustomerReport = ({customer,handleClose}:any) => {
      ...
      const fetchSales = async () => {
        try {
          const response = await userApi.get(`/sales/customer_report/${_id}/${customer._id}`);
          const { status, sales } = response.data;
    
          if (!status) {
            return toast.error("Failed to get customer sales", {
              position: "top-center",
              autoClose: 5000,
              theme: "dark",
            });
          ...
          toast.error("An error occurred while fetching customer sales", {
            position: "top-center",
            autoClose: 5000,
            theme: "dark",
          });
      ...
      return (
        <div className="p-6 bg-gray-800">
          <h1 className="text-2xl font-bold text-white mb-4">Items Report of {customer.name}</h1>
